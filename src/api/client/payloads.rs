...
pub struct MFAStep<'a> {
    pub client: Client<'a>,
    pub user: User,
    pub response: mfa::Response
}

impl<'a> Encodable for MFAStep<'a> {

    fn encode<S: Encoder>(&self, encoder: &mut S) -> Result<(), S::Error> {
        encoder.emit_struct("Request", 4, |encoder| {
            try!(encoder.emit_struct_field("client_id", 0, |e| self.client.client_id.encode(e)));
            try!(encoder.emit_struct_field("secret", 1, |e| self.client.secret.encode(e)));
            try!(encoder.emit_struct_field("access_token", 2, |e| self.user.access_token.encode(e)));
            match self.response {
